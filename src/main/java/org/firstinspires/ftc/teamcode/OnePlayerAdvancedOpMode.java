...
@TeleOp(name = "Z-V3 numero uno Player", group = "Linear OpMode")
public class OnePlayerAdvancedOpMode extends LinearOpMode {
    ...
    private static final int armTargetPositionWhileEnteringOrExitingSubmarine = 500; // position for entering and exiting submarine
    ...
    private static final double dustpanServoPositionDumping = 120 / 180.0; // Specimen intake position 2 (converted to 0-1 range)
    private static final double specimenServoPositionClosed  = 50 / 180.0; // Specimen intake position 1 (converted to 0-1 range)
    private static final double specimenServoPositionOpen  = 120.0 / 180.0; // Specimen intake position 2 (converted to 0-1 range)
    ...
    private void updateRobotState() {
        ...
        if (isCalibrating && !gamepad1.back && calibrationTimer.seconds() >= CALIBRATION_WAIT_TIME) {
        ...
        switch (currentState) {
            ...
            case INTAKEPOSITION1:
                if (gamepad1.left_bumper) {
                    ...
                    configureState(armTargetPositionIntaking, true, liftMinimumEncoderPosition, true, -gamepad1.left_trigger*Math.abs(gamepad1.left_trigger), false, true); // Spinner on
            ...
            case TRANSFERPOSITION:
                if (gamepad1.left_bumper) {
                    ...
                    configureState(armTargetPositionWhileDepositingIntoTheDustPan, true, liftMinimumEncoderPosition, true, gamepad1.left_trigger*Math.abs(gamepad1.left_trigger), false, true);
                } else {
                    configureState(armTargetPositionWhileDepositingIntoTheDustPan, true, liftMinimumEncoderPosition, true, 0, false, true);
                }
            ...
            case LIFTED:
                configureState(armTargetPositionParking, true, liftTopStageEncoderPosition, true, 0, gamepad1.left_trigger > 0.2, true); // Lift raised
                break;
        }
    }
    ...
    private void doMovement() {
        ...
        dustpanServo.setPosition(dustpanState ? dustpanServoPositionDumping : dustpanServoPositionIntaking);


        // Manual specimen/claw control using dpad up/down
        if (gamepad1.a && !lastDpadUp) {
            specimenState = !specimenState;
            servoToggleTimer.reset();
        }
        lastDpadUp = gamepad1.a; //change from dpadup to a
        ...
        if (isCalibrating && gamepad1.back) {
    ...
    private void mecanumDrivetrain() { // Handles controller inputs and setting drivetrain power (almost polished)
        ...
        reducedSpeed = (gamepad1.right_bumper || gamepad1.right_bumper); // Check for right bumper pressed
    ...
    private void telemetryUpdateVerbose() {
        ...
        telemetry.addData("controller position", gamepad1.left_trigger);
    ...
    private void configureState(double armPos, boolean armPIDState, double liftPos, boolean slidePIDState, double spinnerSpeed, boolean dustpanPos, boolean specimenPos) {
        if (gamepad1.b && currentState == RobotState.INTAKEPOSITION1) {
            currentArmTarget = (int) armPos + 20;  // Increase arm setpoint by 20
        } else if (gamepad1.b && (currentState == RobotState.LIFTED ||
                currentState == RobotState.DUMPING ||
